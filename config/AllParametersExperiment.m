...
rho_P=1;  %Radius of the protected area
...
for i=1:NA
    ...
    rA(:,i)=rA0+[RA*cos(thetaA*pi/180), RA*sin(thetaA*pi/180);]';
    vA(:,i)=-(rA(:,i)-rP)*0.01*v_maxA(i)/norm(rA(:,i)-rP);%+rand(2,1);
    %vA(:,i)=v_maxA(i)*([-1,-1]'+2*rand(2,1));
    XA0(:,i)=[rA(:,i);vA(:,i)];
end
%rA = [2.455, -34.5433;6.1548, -33.0553]';
rA =[6.1548, -33.0553]';
vA=zeros(2,NA);
XA0=[rA;vA];
rA_follow=rA-rA(:,1);
...
global rho_sn
rho_Acon=1.3*RA0; %+rhoA_safe;  %radius of the connectivity region
...
rVO={[60,310;160,310;160,390;60,390;]',[-300,350;-190,350;-190,560;-300,560]'};
...
R_m_AD=2*(rho_A+rho_D)+rhoA_safe; %rhoA_safe should have been rhoAD_safe
...
%rD0=[-120,-220]';
%
%rD = [11.9907,-10.7580;  4.3433, -8.3136;   9.3724, -2.1278;   16.1838, -5.8411]';
rD = [11.9907,-10.7580; 9.3724, -7.1278;   16.1838, -5.8411]';
%rD = [11.9907,-10.7580;  4.3433, -8.3136;];
XD0=[rD;zeros(2,ND)];
