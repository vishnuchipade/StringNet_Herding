function [Path,tanG_prime,rE_dash] = findShortestPath(tanG,rI,rF)
% ------------------------------
...
for k=1:NO
    ...
    for i=1:nVO(k)
        ...
        if abs(mL2)<1e16
            ...
            y_int=mL1*rVO{k}(1,i)+cL1;
             lambda1=(y_int-r1(2))/dry;
             dry2=rVO{k}(2,i+1)-rVO{k}(2,i);
            lambda2=(y_int-rVO{k}(2,i))/dry2;
        end
...
for rp=1:2
    ...
    for kt=1:size(tangent_rPO,2)
        plot([rP(1),tangent_rPO(1,kt)],[rP(2),tangent_rPO(2,kt)],'--','color',colors{8+rp});
...
for i=1:size(rTO_all,2)
   text(rTO_all(1,i),rTO_all(2,i),[num2str(i)],'color',colors{3}) 
end
end
    ...
    if G_pathType(v1,v2)==3  %Tangent segments
        ...
        if G_pathType(v1,v2)==1       %1 for anticlockwise segment on the boundary,     
            if g1<=g2
                ...
                ind1=[find(g1<GammaOk{k} & GammaOk{k}<gammaMax); find(0<=GammaOk{k} & GammaOk{k}<g2)];
            end            
