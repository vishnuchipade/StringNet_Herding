function [ uD, arr_EDO,e_vD_des ] = controlDefender(XD,indDef, XD_des, XD_des_dot, XA,pairDA, refTraj, ND )
% ------------------------------
% Author: Vishnu S. Chipade
% Email: vishnuc@umich.edu
% ------------------------------

%this cotrol design is for the modified double integrator dynamics with
%drag term
...
global rO nO aO bO kr0 kr1 kr2 kv1 kv2 RD_con v_maxD u_maxD C_d rho_c_A
...
for j=1:ND
    ...
    if(1)
        for k=1:NO
            ...
            a=aO(k);
            b=bO(k);
            R_OD=norm(rD-rO(1:2,k));
            EmDO=E_m_DO(j,k);
            EbarDO=E_bar_DO(j,k);
            EuDO=E_u_DO(j,k);
            EvDO=E_v_DO(j,k);
            E_DOk=(abs((rD(1)-rO(1,k))/a))^(2*n)+(abs((rD(2)-rO(2,k))/b))^(2*n)-1;
            E_DOk_dot=2*n*[(abs((rD(1)-rO(1,k))/a))^(2*n)/(rD(1)-rO(1,k)), (abs((rD(2)-rO(2,k))/b))^(2*n)/(rD(2)-rO(2,k))]*vD;
    ...
    for i=1:NA
        ...
        if R_DAi<=rho_c_A
            if R_DAi<R_u_AD
                ...
                Rji=R_bar_AD;
                nabla_rj_Vji=nabla_rj_Vji+ka*(rD-rA(1:2,i))/R_DAi*((R_DAi-R_bar_AD)^2-Rji^2)/((R_DAi-R_bar_AD)^2+Rji^2);
...
%             sigmaSumD=sigmaSumD+sigma;
    ...
    if norm_F_D~=0
        ...
        e_vD_des=vD-(sigmaBarProd*(vD_des)+sigmaBarSum*kr1*D_hat*10);
        %e_vD_des=sigmaBarProd*(vD_des+kr1*D_hat*norm_e_rD^(kr2))+sigmaBarSum*kr1*D_hat*norm_e_rD^(kr2);
        %e_vD_des=vD-vD_des;
        
        
        if (1)%norm_e_rD > R_u_AD || abs(-e_rD'*refTraj(1:2,j))<0.7*norm_e_rD*norm(refTraj(1:2,j))
            duD=-.7*e_vD_des*norm( e_vD_des)^(kv2-1)+0.1*sigmaBarProd*D_hat*norm_e_rD^(kr2)+C_d*(vD-e_vD_des);
           % duD=-.7*e_vD_des*norm( e_vD_des)^(kv2-1)+uDA;
        else
            e_vD_des=vD-refTraj(1:2,j)*vACon(1:2,j)/norm(refTraj(1:2,j));
            Rji=Rji0(j);
            Rj_i=norm(rACon(1:2,j)-rD);
            R_DAi=Rj_i;
            uDA= ka*(rD-rACon(1:2,j))/R_DAi*((R_DAi-R_bar_AD)^2-Rji^2)/((R_DAi-R_bar_AD)^2+Rji^2); %just for checking, original control is defined above
            duD = -.7*e_vD_des*norm( e_vD_des)^(kv2-1)+uDA;
        end
