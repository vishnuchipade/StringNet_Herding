function [results] = startTimeForCollisionAvoidance2(inpStruct)
% ------------------------------
...
%collide,
%This is based on necessary and sufficient conditions i.e. no two robots
%collide at any time (the lead times are calculated separately using
%bisection method)
...
for j=1:ND
    for jj=j+1:ND
        if interSec{j,jj}.flag0==1
            ...
            A(ND+2*ci-1, 1+j)=1;
            A(ND+2*ci-1, 1+jj)=-1;
            b(ND+2*ci-1,1)= interSec{j,jj}.T21-min(interSec{j,jj}.T11+leadTime{j,jj}.T1,interSec{j,jj}.T11);
            A(ND+2*ci, ci0+ci)=M;
            A(ND+2*ci, 1+j)=-1;
            A(ND+2*ci, 1+jj)=1;
            b(ND+2*ci,1)= -min(interSec{j,jj}.T21+leadTime{j,jj}.T2,interSec{j,jj}.T22)+interSec{j,jj}.T11+M;            
        end
    end    
end
