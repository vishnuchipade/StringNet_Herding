%This script simulates herding of a group of attackers by a swarm of
%defenders
...
%Load parameters from a separate file depending on the type of simulation
flagExp=0;
flagGazeboExp=0;
if flagExp==1
...
Cov_YA=diag([0,0,0,0]);
elseif scenario==2
Cov_YA=diag([(0.9/3)^2,(.9/3)^2,(.12)^2,(.12)^2]);
%Cov_YA=diag(0*[(3)^2,(3)^2,(2)^2,(2)^2]);
elseif scenario==3
Cov_YA=diag([0,0,0,0]);
end
...
[WA, Rii_bar]=findCommGraphAndFormDist(NA,1,RA);  %second argument is shapeId:
...
R_DD_string=1.5*Rjj_bar_closed(1,2);
...
if(flagTanGraph)
    ...
    obs.rCO2=rCO2;
    if flagExp==1
        save('data/tempDataExp.mat','obs','tanG','-append');
    elseif flagGazeboExp==1
        save('data/tempDataGazeboExp.mat','obs','tanG','-append');
    else
        save('data/tempData.mat','obs','tanG','-append');
    end
else
    if flagExp==1
        load('data/tempDataExp.mat');
    elseif flagGazeboExp==1
        load('data/tempDataGazeboExp.mat')
    else
        load('data/tempData.mat');
    end
    flagTanGraph=0;
    rCO2=obs.rCO2;
end
...
if flagMotionPlan
    ...
    if flagExp==1
        save('data/tempDataExp.mat','motionPlan','defDesForm','-append');
    elseif flagGazeboExp==1
        save('data/tempDataGazeboExp.mat','motionPlan','defDesForm','-append');
    else
        save('data/tempData.mat','motionPlan','defDesForm','-append');
    end
end
...
%Start the simulation loop for Niter time steps
for ti=1:Niter
    if mod(ti,1000)==0
        ti
    end
    ...
    XA_goal_dot(:,1)=[XA_lead_goal(1:2,1);0;0];
    ...
    for i=2:NA
        ...
        XA_goal_dot(:,i)=[XA(3:4,i); uA(1:2,1)];
        if(~flagAttackerStayTogether)
         if flagEnclose==1 || flagHerd==1
            if i>NA-NA_sep
                XA_goal(:,i)=XA_lead_goal+[rA_follow(1:2,i);0;0];
    ...
    %when some of the attackers don't stay together
    [uA, uA0, R_AO_min, R_AAProjS_min, vA_des, vA_des_dot, rAProj1, flagAttInObs, betaAv0, speedA0, mAv0, cAv0, SigmaProdD, FA, FA_dot] = controlAttacker4(XA,XA_goal,XA_goal_dot,vA,flagAttInObs,flagEnclose,flagHerd,betaAv0,speedA0,mAv0,cAv0,XD,WA,WDString,NA,ND);
    ...
    if flagGather==1 && flagSeek~=1 && flagEnclose~=1 && flagHerd~=1
        ...
        for j=1:ND
            if norm(XD(1:2,indDef(j))-XD_des(1:2,j))<1e-3 %&& norm(XD(3:4,j))<1e-8 && flagDefReachOpen~=1
                defReachCount(j)=1;
    ...
    elseif flagGather~=1 && flagSeek~=1 && flagEnclose==1 && flagHerd~=1 %flagHerd~=1  && flagDefConnect~=1 %countDDes<ND  %Enclosing phase
        ...
        j1=find(assign==1); %Defender corresponding to first desired position
        jND=find(assign==ND);   %Defender corresponding to last desired position
        if (norm(XD(1:2,j1)-XD_des(1:2,1))< bd && norm(XD(1:2,jND)-XD_des(1:2,ND))<bd) ||  (norm(XD(1:2,j1)-XD(1:2,jND))<=0.95*R_DD_string && flagAttackInHull)
            WDString(j1,jND)=1;
            WDString(jND,j1)=1;
        else
            WDString(j1,jND)=0;
...
%                  XD_des_dot=XD_des_dot(:,assign);
    ...
    %Collect all inputs
    U(:,ti)=[uA(:);uD(:)];
    %Integrate dynamics of all players
    X(:,ti+1)=modifiedDIDynamics(X(:,ti),U(:,ti));
    %Collect states of attackers
    XA=reshape(X(1:4*NA,ti+1),[4,NA]);
    %Generate measurements of attackers
    YA=XA+mvnrnd(zeros(4,1),Cov_YA,NA)';
    %Positions of attackers
    rA=XA(1:2,:);
    %Velocities of attackers
    vA=XA(3:4,:);
    %Collect states of defenders
    XD=reshape(X(4*NA+1:4*(N+1),ti+1),[4,ND+1]);
    %Generate measurements of defenders
    XDp=reshape(X(4*NA+1:4*(N+1),ti),[4,ND+1]);  %previous state
    %Positions of defenders
    rD=XD(1:2,:);
    %Velocities of defenders
    vD=XD(3:4,:);
    ...
    for j=1:ND
        ...
        for k=1:NO
            ...
            arr_EDO_crit(j,k)=(R_m_DD-rhoD_safe)/arr_EDO(j,k);
        end
    end
    if ND>1
    ...
    if NA>1
        minEAO(ti)=(R_m_AA-rhoA_safe)/(R_AO_min);
    end
    minRAAProjS(ti)=(R_m_AD-rhoAD_safe)/R_AAProjS_min;
...
fontSize=18;
fig=figure;
plotTraj(X(:,1:ti),rP,rS,rVO,rho_S,time(1:ti),NA,ND,'o','--',fig.Number,1,1,1);
set(gca,'fontsize',fontSize)
%%
%animateTraj(X,rP,rS,rVO,rho_Acon,rho_S,t1,length(time),dt,phase_time,NA,ND,XD_Des,WDString_mat);
%%
for i=1:NA+ND
    colors{i}=rand(1,3);
end
...
if(1)
    ...
    sp1.Position(4)=0.5;
    for j=1:N
        plot(time(1:ti),sqrt(U(2*(j-1)+1,1:ti).^2+U(2*(j-1)+2,1:ti).^2),'color',colors{j},'linewidth',1.5);
        hold on;
        if j<NA+1
    ...
    plot(time(1,1:ti),u_maxD(1)*ones(1,ti),'color',[1,0,0],'linestyle','--','linewidth',1.5)
    plot(time(1,1:ti),u_maxA(1)*ones(1,ti),'color',[0,0,1],'linestyle','--','linewidth',1.5)
...
%     text(dt*ti_g/10,1.4*u_maxD(1),'$Gathering$','fontsize',fontSize,'color','c')
    ...
    leg = legend(legends,'orientation','horizontal','interpreter','latex')
    ...
    sp1.Position(2)=0.07+0.3;
    sp1.Position(4)=0.5;
    plot(time,(R_m_DD-rhoD_safe)./minRDD,'b','linewidth',1.5)
    hold on
    plot(time,(R_m_AD-rhoAD_safe)./minRAD,'k','linewidth',1.5)
    hold on
    plot(time,(R_m_AA-rhoA_safe)./minRAA,'r','linewidth',1.5)
    plot(time,minEDO,'c','linewidth',1.5)
    hold on
    plot(time,minEAO,'m',time,ones(size(time)),'r--','linewidth',1.5)
    
    ylabh=ylabel('Distance Ratios')
    ...
    leg=legend('$\Delta_{d}^{d}$','$\Delta_{d}^{a}$','$\Delta_{a}^{a}$','$\Delta_{d}^{o}$','$\Delta_{a}^{o}$','y=1','orientation','horizontal','interpreter','latex')
    set(gca,'fontsize',fontSize)
    leg.ItemTokenSize =[14,10];
    leg.Position(2)=0.9;
    ylim([0,1.2])
    set(ylabh,'Units','normalized');
    set(ylabh,'position',get(ylabh,'position') - [0.05 0.24 0]);
end
...
if(0)
    ...
    figure
    %subplot(2,1,1)
    plot(time,(R_m_DD-rhoD_safe)./minRDD,'b')
    %ylabel('$\displaystyle \max_{j\neq l \in I_d} \frac{R_{dd}^m}{||\textbf{r}_{dj}-\textbf{r}_{dl}||}$')
    %subplot(2,1,2)
    hold on
    plot(time,(R_m_AD-rhoD_safe)./minRAD,'r',time,ones(size(time)),'r--')
