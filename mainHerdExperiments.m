...
for i=1:NA
    ...
    formatspec1 = '/attacker%d/mavros/local_position/odom';     
    formatspec2 = '/attacker%d/station/pose';
    formatspec3 = '/attacker%d/localENUoffset';
   
    
    string = sprintf(formatspec1,i);
    string1 = sprintf(formatspec3,i);
    subA_odometry(i) = rossubscriber(string,'nav_msgs/Odometry');
   
        subA_offset(i)= rossubscriber(string1,'geometry_msgs/PoseStamped');
 
    string = sprintf(formatspec2,i);
    subA_pose(i) = rossubscriber(string,'geometry_msgs/PoseStamped');
...
for j=1:ND
    ...
    formatspec1 = '/defender%d/mavros/local_position/odom';     
    formatspec2 = '/defender%d/station/pose';
    formatspec3 = '/defender%d/localENUoffset';
   
    
    %formatspec = '/pelican/odometry_sensor1/odometry';
    string = sprintf(formatspec1,j);
    
    string1 = sprintf(formatspec3,j);
    subD_odometry(j) = rossubscriber(string,'nav_msgs/Odometry');
   
    subD_offset(j)= rossubscriber(string1,'geometry_msgs/PoseStamped');

     
    %formatspec = '/pelican/odometry_sensor1/pose';
    string = sprintf(formatspec2,j);
    subD_pose(j) = rossubscriber(string,'geometry_msgs/PoseStamped');
...
sumDy=0;
%XD0=zeros(4,ND);
while flagA==1 || flagD==1
    for i=1:NA
        NewAmsg=subA_pose(i).LatestMessage;
        X_a(i)=NewAmsg.Pose.Position.X;
        Y_a(i)=NewAmsg.Pose.Position.Y;
        if X_a(i) || Y_a(i)
            flagA=0;
        else
            flagA=1;
        ...
        sumX=sumX+X_a(i);
        sumY=sumY+Y_a(i);
        YA(1:4,i) =[X_a(i), Y_a(i), 0, 0]';
        
        NewAmsg=subA_offset(i).LatestMessage;
        Xoff_a(i)=NewAmsg.Pose.Position.X;
        Yoff_a(i)=NewAmsg.Pose.Position.Y; 
        pause(1)        
    end
    pause(.2)
    for i=1:ND
        NewDmsg=subD_pose(i).LatestMessage;
        X_d(i)=NewDmsg.Pose.Position.X;
        Y_d(i)=NewDmsg.Pose.Position.Y;
        if X_d(i) || Y_d(i)
            flagD=0;
        else
            flagD=1;
        ...
        sumX=sumX+X_d(i);
        sumY=sumY+Y_d(i);
        XD0(1:4,i) =[X_d(i), Y_d(i), 0, 0]';
        
        NewAmsg=subD_offset(i).LatestMessage;
        Xoff_d(i)=NewAmsg.Pose.Position.X;
        Yoff_d(i)=NewAmsg.Pose.Position.Y;
        pause(1)
    end
end
%XD = XD0;
%X(:,1)=[YA;XD0(:);zeros(4,1)];
...
startTime=motionPlan.startTime+ones(length(motionPlan.startTime),1)*25;
clear motionPlan;
clear tanG;
...
while flagHover 
    ...
    theta=0.5*t;
    for i=1:NA
        msgPubA_pose(i).Pose.Position.X=X_a(i);
        msgPubA_pose(i).Pose.Position.Y=Y_a(i);
        msgPubA_pose(i).Pose.Position.Z=z_h;
        send(pubA_pose(i),  msgPubA_pose(i))
    end
    for i=1:ND
        msgPubD_pose(i).Pose.Position.X=X_d(i);
        msgPubD_pose(i).Pose.Position.Y=Y_d(i);
        msgPubD_pose(i).Pose.Position.Z=z_h;
        send(pubD_pose(i),  msgPubD_pose(i))
    end
    %pause(0.01);
end
...
sumT=0;
disp('Starting main herding loop')
for ti=1:Niter
    %tic
    Psi=zeros(NA,1);
    Psi_dot=Psi;
    ...
    elseif flagGather~=1 && flagSeek==1 && flagEnclose~=1 && flagHerd~=1
        ...
        %Due to limited space ignore the seeking phase and directly start
        %enclosing phase
        flagEnclose=1;
            flagSeek=0;
        
        if (norm(rAcm-rDcm)<(2.5*rho_sn) && flagAttInSight) %&& flagHerd~=1 && flagDForm~=1 %&& norm(vAcm)>0.1
            flagEnclose=1;
            flagSeek=0;
        end
        ti_s=ti;
    elseif flagGather~=1 && flagSeek~=1 && flagEnclose==1 && flagHerd~=1 %flagHerd~=1  && flagDefConnect~=1 %countDDes<ND  %Enclosing phase
        ...
        j1=find(assign==1); %Defender corresponding to first desired position
        jND=find(assign==ND);   %Defender corresponding to last desired position
        if (norm(XD(1:2,j1)-XD_des(1:2,1))< bd && norm(XD(1:2,jND)-XD_des(1:2,ND))<bd) ||  (norm(XD(1:2,j1)-XD(1:2,jND))<=0.97*R_DD_string && flagAttackInHull)
            WDString(j1,jND)=1;
            WDString(jND,j1)=1;
        else
            WDString(j1,jND)=0;
...
%                 msgPubA_pose(i).Pose.Orientation.W=quat(1);
        ...
        for i=1:NA
            ...
            pos(:,i)=[msgSubA_pose.Pose.Position.X, msgSubA_pose.Pose.Position.Y, msgSubA_pose.Pose.Position.Z]';
        ...
        for j=1:ND
            ...
            pos(:,jj)=[msgSubD_pose.Pose.Position.X, msgSubD_pose.Pose.Position.Y, msgSubD_pose.Pose.Position.Z]';
...
%     end
    ...
    XA=[rA;vA];
    XD=[rD;vD];
    %X(4*NA+1:end,ti+1)=XD(:);  %reset the current position for simulation
    %from true position
