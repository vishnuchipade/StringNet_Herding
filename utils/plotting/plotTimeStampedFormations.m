...
ti1=1;
ti2=ti_g;
figNumber=plotTraj(X(:,ti1:ti2),rP,rS,rVO,rho_S,time(ti1:ti2),NA,ND,'o','--',fig.Number,1,1,0);
...
NA4=4*NA;
axis equal
dXD0=[-50,10;-10,-40;20,-15;13,45;-60,20]';

for j=1:ND
    plot(X(NA4+4*j-3,ti_g),X(NA4+4*j-2,ti_g),'b*','markersize',markSize)
    text(XD_des0(1,j)+dXD0(1,j),XD_des0(2,j)+dXD0(2,j),['$\mathcal{\xi}_',num2str(j),'^g$'],'fontsize',fontSize)%,'color',[0,0,1])
...
assignStr(end-1:end)=[];
%str={['$\circ \;$ - Initial positions'],[ '$\ast \; $ - Gathering (@ t= ',num2str(dt*ti_1),' s)'], ['Assignment: [', assignStr, ']']};
str={['$\circ \;$  Initial positions'],['$-- \;$ Paths during Gathering'],[ '$\ast \; $ Gathering (@ t= ',num2str(dt*ti_1),' s)']};

annotation('textbox',[.55 .235 .29 .115],'string',str,'fontsize',fontSize,'interpreter','latex')
...
str={['$- \;$ Paths during Seeking'],['$\diamond \;$ Seeking (@ t= ',num2str(dt*ti_s),' s)']};

annotation('textbox',[.63 .14 .27 .08],'string',str,'fontsize',fontSize,'interpreter','latex')

%%
%Gathering + Seeking + Enclosing phase
%Gathering phase
%Plot the trajectories
fontSize=18;
% ti1=1;
% ti2=ti_g;
% figNumber=plotTraj(X(:,ti1:ti2),rP,rS,rVO,rho_S,time(ti1:ti2),NA,ND,'o','--',0,1);
%figNumber=0;
ti1=ti_g+1;
ti2=ti_s;
%plotTraj(X(:,ti1:ti2),rP,rS,rVO,rho_S,time(ti1:ti2),NA,ND,'-','-',figNumber,0);
...
if (1)
    for i=1:NA
        plot(X(4*i-3,ti_3),X(4*i-2,ti_3),'rp','markersize',markSize)
    end
    NA4=4*NA;
    for j=1:ND
        plot(X(NA4+4*j-3,ti_3),X(NA4+4*j-2,ti_3),'bp','markersize',markSize)
        %line
        %plot([X(NA4+4*j-3,ti_1),X(NA4+4*j+1,ti_1)], [X(NA4+4*j+2,ti_1),X(NA4+4*j+2,ti_1)],'b-')
    end
    i=ti_e+1;
...
assignStr(end-1:end)=[];
% str={[ '$\ast \;$  Gathering (@ t= ',num2str(dt*ti_1),' s)'],['$- \;$ Paths during Seeking'],['$\diamond \;$ Seeking (@ t= ',num2str(dt*ti_s),' s)'],...
%     ['$-. \;$ Paths during Seeking'],['$\star \;$ Closed StringNet (@ t= ',num2str(dt*ti_3),' s)']};
str={['$-. \;$ Paths during Enclosing'],['$\star \;$ StringNet (@ t= ',num2str(dt*ti_3),' s)']};

annotation('textbox',[.6 .12 .3 .08],'string',str,'fontsize',fontSize,'interpreter','latex')

%%

%Gathering + Seeking + Enclosing +herding phase
%Gathering phase
%Plot the trajectories
% fontSize=18;
% ti0=ti_g;
% plotTraj(X(:,1:ti0),rP,rS,rVO,rho_S,time(1:ti0),NA,ND,'--',0,);
% set(gca,'fontsize',fontSize)
% hold on;
...
str={['$.. \;$ Paths during herding'],['$\triangle \;$  Final Positions (@ t= ',num2str(dt*ti),' s)']};

annotation('textbox',[.56 .12 .35 .08],'string',str,'fontsize',fontSize,'interpreter','latex')
