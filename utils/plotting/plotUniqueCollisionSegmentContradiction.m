...
%plot the points
 plot(rI(1,1),rI(2,1),'bo');
 plot(rF(1,1),rF(2,1),'bsquare');
 plot(rI(1,2),rI(2,2),'ro');
 plot(rF(1,2),rF(2,2),'rsquare');



%Find the shortest paths and plot
[Path1,tanG_prime1,rE_dash1] = findPath(tanG,rI(:,1),[10;7],2,[]);
PlotShortestPath(tanG,Path1,tanG_prime1,1,[0,0,1],'--')
%path 1
[Path1,tanG_prime1,rE_dash1] = findPath(tanG,rI(:,1),rF(:,1),2,[]);
PlotShortestPath(tanG,Path1,tanG_prime1,1,[0,0,1],'--')

%path2
[Path2,tanG_prime2,rE_dash2] = findPath(tanG,rI(:,2),rF(:,2),3,[]);
PlotShortestPath(tanG,Path2,tanG_prime2,1,[1,0,0],'--')

[Path01,tanG_prime01,rE_dash01] = findPath(tanG,rI(:,1),rF(:,1),0,[9,11,15,10]);
PlotShortestPath(tanG,Path01,tanG_prime01,1,[0,0,1],'-')

[Path02,tanG_prime02,rE_dash02] = findPath(tanG,rI(:,2),rF(:,2),0,[9,12,15,10]);
PlotShortestPath(tanG,Path02,tanG_prime02,1,[1,0,0],'-')


%%
hold on
text(rI(1,1),rI(2,1)-.7,'$\mathbf{r}_{11}$','fontsize',fontSize)
text(rF(1,1),rF(2,1)+.7,'$\mathbf{r}_{12}$','fontsize',fontSize)

text(rI(1,2),rI(2,2)-.7,'$\mathbf{r}_{21}$','fontsize',fontSize)
text(rF(1,2),rF(2,2)+.7,'$\mathbf{r}_{22}$','fontsize',fontSize)
